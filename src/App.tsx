...
export default function App() {
  ...
  }) as CSSProperties, [settings.fontFamily, settings.fontSize]);

  const activeWorkspaceLabel = selectedWorkspace
  ...
  return (
    ...
    >
      <div className="app-frame">
        <section className="surface-panel shell-header">
          <div className="shell-header-main">
            <div className="stack-sm">
              <div>
                <p className="brand-kicker">Operator Workspace</p>
                <h1 className="brand-title">Knuthflow</h1>
                <p className="brand-subtitle">
                  Keep agent sessions, workspace context, and autonomous runs readable enough to steer at a glance.
                </p>
              </div>
              <div className="shell-header-meta">
                <span className="operator-chip">
                  <span className={`status-dot ${loading ? 'warning' : status?.installed ? 'success' : 'danger'}`} />
                  <strong>
                    {loading ? 'Checking Claude Code' : status?.installed ? `Claude Code ${status.version || 'ready'}` : 'Claude Code missing'}
                  </strong>
                </span>
                <span className="operator-chip">
            ...
            <div className="toolbar-cluster">
              ...
              {updateInfo?.available && updateInfo.downloadUrl && (
                ...
                >
                  <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-5l-4 4m0 0l-4-4m4 4V4" />
                  </svg>
              ...
              {!loading && status?.installed && viewMode === 'terminal' && (
                activeRun && (activeRun.state === 'starting' || activeRun.state === 'running') ? (
                  <button onClick={handleStopClaude} className="btn btn-danger">Stop Session</button>
                ) : (
                  <button onClick={handleStartClaude} className="btn btn-primary">New Session</button>
                )
              )}
            </div>
          </div>
          ...
          <div className="toolbar-inline">
            ...
            {!status?.installed && !loading && (
              <span className="badge badge-danger">Install Claude Code to launch sessions</span>
            )}
          </div>
        </section>
        ...
        <main className="surface-panel main-stage">
